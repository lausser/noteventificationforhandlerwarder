...
The `notificationforwarder` system MUST be able to publish a formatted monitoring notification as a signed Nostr text note to one or more configured relays, and MUST accept recipient `p` tag public keys as either `npub1...` strings or raw hex values before relay submission.
...
#### Scenario: Recipient public key is provided as npub
- **WHEN** a published Nostr note contains a `p` tag with an `npub1...` recipient public key
- **THEN** the system MUST convert the key to raw hex before submitting the event to relays

#### Scenario: Recipient public key is provided as hex
- **WHEN** a published Nostr note contains a `p` tag with a raw hex recipient public key
- **THEN** the system MUST preserve the key value unchanged before submitting the event to relays
